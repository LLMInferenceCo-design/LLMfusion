...
def process_config(args):
    """单独处理一个硬件配置的函数，用于多进程"""
    hardware_config,  i, test_function = args
    ...
    print(f"Config: {config[i]} pass")
    return config[i], area, latency  # 返回更多信息
...
if __name__ == "__main__":
    ...
    for i in range(len(config)-1):
        task.append((copy.deepcopy(hardware_config), i, change_config_prefill_test))
    with Pool(processes=cpu_count()) as pool:
        results = pool.map(process_config, task)

    for result in results:
        config_name, area, latency = result
        print(f"Config: {config_name}, Area: {area}, Latency: {latency}")

    csv_file_path = "../ae/res2.xlsx"
    sheet_name = "prefill"
    save_to_xlsx(csv_file_path, sheet_name, results)

    task =[]
    for i in range(len(config)):
        task.append((copy.deepcopy(hardware_config), i, change_config_decode_test))
    with Pool(processes=cpu_count()) as pool:    
        results = pool.map(process_config, task)
    for result in results:
        config_name, area, latency = result
