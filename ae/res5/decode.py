...
config = ["LLMCompass_Latency", "LLMCompass_Throught", "GA100", "me_decode"]
core_count = [64, 64, 128, 128]
sublane_count = [4, 4, 4, 4]
vector_width = [32, 32, 32, 16]
array_height = [16, 32, 16, 16]
array_width = [16, 32, 16, 16]
SRAM_KB = [192, 768, 192, 128]
global_buffer_MBS = [24, 48, 48, 24]

global_buffer_bandwidth_per_cycle_bytes = [2560, 5120, 5120, 7680]
memory_bandwidths = [5, 2.5, 5, 5]
total_capacity_GBS = [80, 512, 80, 80]
memory_protocols = ["HBM2e", "PCIe5", "HBM2e", "HBM2e"]
device_count = 4
...
if __name__ == "__main__":
    ...
    sheet_name = "decode"
    save_to_xlsx(csv_file_path, sheet_name, results, Lout, batch_size, config)
    end_time = time.time()
    logger.debug(f"Prefill test time: {end_time - start_time} seconds")
