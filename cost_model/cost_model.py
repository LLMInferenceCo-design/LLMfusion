...
# from cost_model.supply_chain import supply_chain_model as scm
...
def calc_cache_sram_area_mm2(capacity_bytes, sram_bitcell_area_mm2, max_cache_unit_bytes=(2**19)):
    ...
    area_efficiency_index = math.ceil(math.log(unit_size_bytes, 2)) - 12 #这个实际就是最高位的index
...
def calc_compute_chiplet_area_mm2(configs_dict, verbose=False):
    ...
    per_sublane_area_mm2 = 0 #这个主要包括control logic, vector unit, systolic array,register file
    ...
    cache_area_mm2 = calc_cache_sram_area_mm2(cache_size_bytes, sram_density_bitcell_mm2)#这里面cache指的就是local buffer
    per_core_area_mm2 = per_core_compute_area_mm2 + cache_area_mm2

    core_breakdown_map['total_core_area'] = per_core_area_mm2
    core_breakdown_map['control_area'] = control_logic_area #这个是control指的应该就是白皮书里的warp和Dispatch
    ...
    compute_chiplet_area_mm2 = total_cores_area + total_crossbar_area #Crossbar是SM之间通信用的，白皮书里没有给
    ...
    if verbose:
        ...
        return compute_chiplet_area_mm2#这个是整个chiplet的面积，包括core、crossbar，但是不包括Global buffer可能要后面计算
...
def calc_io_die_area_mm2(config_dict, verbose=False):
    ...
    cache_size_bytes = config_dict['device']['io']['physical_global_buffer_MB'] * (2 ** 20) #这个是global buffer
    mem_tech = config_dict['device']['memory_protocol']
    #
    num_mem_tech_units = config_dict['device']['io']['memory_channel_physical_count']#这个好像是L2cache和HBM2之间的controller数,论文中的Mem PHY
    gpu_gpu_comm_tech = config_dict['interconnect']['link']['name']
    num_gpu_gpu_comm_phy = config_dict['interconnect']['link_count_per_device']#这玩意指的是论中的Device PHY
    ...
    mem_controller_area = calc_mem_controller_area_mm2(mem_tech, num_mem_tech_units, transistor_density_mil_mm2)#白皮书里面的memory controller
    ...
    device_phy_area = calc_mem_phy_area_mm2(gpu_gpu_comm_tech, num_gpu_gpu_comm_phy)#12个NVlink通道面积
    device_controller_area = calc_mem_controller_area_mm2(gpu_gpu_comm_tech, num_gpu_gpu_comm_phy, transistor_density_mil_mm2)#12个NVlink控制器面积
